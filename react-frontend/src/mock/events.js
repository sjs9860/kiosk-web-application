...
export async function getEvents(query) {
  ...
  if (!events) events = defaultEvents.map(e => ({...e, id: Math.random().toString(36).substring(2, 9)}));
  if (query) {
    events = matchSorter(events, query, { keys: ["title"] });
  }
...
export async function createEvent(data) {
  ...
  let event = { id, createdAt: Date.now(), ...data };
